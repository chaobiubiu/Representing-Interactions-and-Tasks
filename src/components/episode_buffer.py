...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        if preprocess is not None:
            for k in preprocess:
                assert k in scheme                  # k: actions
                new_k = preprocess[k][0]            # actions_onehot
                transforms = preprocess[k][1]       # OneHot(out_dim=args.n_actions)
                ...
                self.scheme[new_k] = {
                    "vshape": vshape,
                    "dtype": dtype
                }                           # "actions_onehot": {"vshape": (args.n_actions, ), "dtype": torch.float32}
                if "group" in self.scheme[k]:
                    self.scheme[new_k]["group"] = self.scheme[k]["group"]       # "actions_onehot": {"vshape": (args.n_actions, ), "group": "agents", "dtype": torch.float32}
        ...
        for field_key, field_info in scheme.items():
            ...
            if group:
                ...
                shape = (groups[group], *vshape)            # The shapes of obs, actions, avail_actions, probs, actions_onehot are (args.n_agents, v_shape)
            else:
                shape = vshape
    ...
    def max_t_filled(self):
        return th.sum(self.data.transition_data["filled"], 1).max(0)[0]     # shape=(1)
