...
# TODO: 需要定义一个新的mixing network，其中包括task rep的mapping function以及基于attention机制的hypernet，之前环境文件中定义的entity_mask在这里产生作用
...
class Mixer(nn.Module):
    ...
    def forward(self, qvals, states):
        # qvals.shape=(bs,(max_seq_length-1),n_agents), states.shape=(bs,(max_seq_length-1),state_size)
        # reshape
        b, t, _ = qvals.size()
        ...
        # First layer, w1: (bs*t, state_dim) --> (bs*t, self.n_agents*self.embed_dim) --> (bs*t, n_agents, embed_dim)
        w1 = self.hyper_w1(states).view(-1, self.n_agents, self.embed_dim) # b * t, n_agents, emb
        b1 = self.hyper_b1(states).view(-1, 1, self.embed_dim)  # (bs*t, 1, embed_dim)
        
        # Second layer, w2: (bs*t, state_dim) --> (bs*t, self.embed_dim, 1)
        w2 = self.hyper_w2(states).view(-1, self.embed_dim, 1) # b * t, emb, 1
        b2= self.hyper_b2(states).view(-1, 1, 1)        # (bs*t, 1, 1)
