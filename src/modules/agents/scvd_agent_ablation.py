...
class ObsEncoder(nn.Module):
    def __init__(self, input_dim, args):
        ...
        # q_alone
        self.q_alone = nn.Linear(args.rnn_hidden_dim, self.n_actions)

    def init_hidden(self):
        # make hidden states on same device as model
        return self.query.weight.new(1, self.args.rnn_hidden_dim).zero_()

    def forward(self, inputs_interact, obs_mask, scenario_mask, h_interactive):
        ...
        '''将attention_out输出和embeded_inputs concat作为rnn网络的输入'''
...
class QValueDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.q_out = nn.Linear((self.hidden_dim + self.atten_dim), self.n_actions)   # \tau^{i}+encoder(a^{-i})+task_rep --> q^{i}

        self.mask = th.tril(th.ones(self.n_agents, self.n_agents)).view(1, self.n_agents, self.n_agents).to(args.device)

    def forward(self, h_interactive, onehot_actions, scenario_mask):
        ...
        concat_inps = th.cat([h_interactive, extra_act_embeddings], dim=-1)      # (bs, n_agents, hidden_dim+atten_dim+task_rep_dim)
        ...
        # TODO: 增加对于q^i(\tau^i, i, a^{-i})的约束, this term should equal to zero.
        # First way
        ...
        auxiliary_q_inps = th.cat([h_interactive, auxiliary_act_embeddings], dim=-1)     # (bs, n_agents, hidden_dim+atten_dim+task_rep_dim)
    ...
    def act_forward(self, curr_inps, curr_query, shifted_action, agent_mask, curr_index):
        ...
        concat_inps = th.cat([curr_inps, extra_act_embeddings], dim=-1)
...
class SCVDRNNAgent_Ablation(nn.Module):
    ...
    def forward(self, inputs, onehot_actions, obs_mask, scenario_mask, hidden_states):
        ...
        all_q_alone, h = self.obs_encoder(inputs, obs_mask, scenario_mask, hidden_states)      # \tau, shape=(bs, n_agents, hidden_dim)
        all_q_interact, auxiliary_q = self.q_decoder(h, onehot_actions, scenario_mask)
        # TODO: 这里是否需要对all_q_interact增加一些约束呢？remaining to be done.
        all_q = all_q_alone + all_q_interact
        return all_q, auxiliary_q, all_q_alone, all_q_interact, h

    def autoregressive_act(self, inputs, obs_mask, scenario_mask, hidden_states, avail_actions, bs_index, t_env, action_selector, test_mode):
        # inputs.shape=(bs, n_entities, local_state_size+n_actions)，包括每个entity的local info以及last action, padded zero last_action for non-agents
        # task_z.shape=(bs, n_agents, task_rep_dim)
        all_q_alone, h = self.obs_encoder(inputs, obs_mask, scenario_mask, hidden_states)
        ...
        for i in range(n_agents):
            ...
            curr_q = self.q_decoder.act_forward(curr_hidden, curr_query, curr_shifted_action, agent_mask, curr_index=i)
