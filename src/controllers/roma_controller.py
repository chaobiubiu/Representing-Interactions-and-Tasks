...
class ROMAMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False, t_glob=0, train_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)  # (bs, n_entities, local_state_size+n_actions)
        avail_actions = ep_batch["avail_actions"][:, t]
        obs_mask = ep_batch["obs_mask"][:, t, :, :]  # (bs, n_entities, n_entities)
        scenario_mask = ep_batch["scenario_mask"][:, t, :]  # (bs, n_entities)
        if test_mode:
            self.agent.eval()
        agent_outs, self.hidden_states, loss_cs, dis_loss, ce_loss = self.agent(agent_inputs, obs_mask, scenario_mask,
                                                                                 self.hidden_states, t=t, batch=ep_batch,
                                                                                 t_glob=t_glob, train_mode=train_mode)
        ...
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), loss_cs, dis_loss, ce_loss
