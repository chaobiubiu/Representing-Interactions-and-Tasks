...
class ROMALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "attn_mix":
                self.mixer = AttnMixer(args)
            else:
                raise Exception("The ROMALearner must use the attn_mix mixer.")
            self.params += list(self.mixer.parameters())
            self.target_mixer = copy.deepcopy(self.mixer)

        self.state_encoder = MultiHeadStateInpEncoder(args)      # states, scenario_mask --> vector, vector.shape=(bs, n_agents, hidden_dim)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        for t in range(batch.max_seq_length):
            agent_outs, loss_, dis_loss_, ce_loss_ = self.mac.forward(batch, t=t, t_glob=t_env, train_mode=True)
            reg_loss += loss_
            dis_loss += dis_loss_
            ce_loss += ce_loss_
        ...
        for t in range(batch.max_seq_length):
            target_agent_outs, loss_cs_target, _, _ = self.target_mac.forward(batch, t=t)
        ...
        if self.mixer is not None:
            if self.args.mixer == "attn_mix":
                ...
                chosen_action_qvals = self.mixer(chosen_action_qvals, mixer_inps[:, :-1], batch["scenario_mask"][:, :-1, :])
                target_max_qvals = self.target_mixer(target_max_qvals, mixer_inps[:, 1:], batch["scenario_mask"][:, 1:, :])
            else:
