...
class Ablation_SCVDLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):    # t_env, episode_num are used to log.
        ...
        for t in range(batch.max_seq_length):
            agent_outs, auxiliary_agent_outs, q_alone, q_interact = self.mac.forward(batch, t=t)
        ...
        '''20221020 如果使用attn_mixer，则将all_task_z与mixer_inps concat作为mixing network的输入'''
        # concat_mixer_inps = th.cat([mixer_inps, all_task_z.detach()], dim=-1)       # (bs, max_seq_length, n_agents, (rnn_hidden_dim + task_rep_dim))
        concat_mixer_inps = mixer_inps      # (bs, max_seq_length, n_agents, rnn_hidden_dim)
