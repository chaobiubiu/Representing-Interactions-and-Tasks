...
class RandomStarCraft2CustomEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.training_tasks = self.scenario_dict['training_scenarios']
        self.testing_tasks = self.scenario_dict['testing_scenarios']
        ...
        # ally_army / enemy_army: (num, unit, ally_pos + (self.rs.rand(2) - 0.5) * 2 * self.pos_jitter)

        # At first, self.n_agents and self.n_enemies are set to the maximum.
        for num, u_name, pos in ally_army:
            ...
            self.stand2cust[stand_utype] = cust_utype           # standard unit type id to custom unit type id
        ...
        self.max_n_agents = self.n_agents       # Set the maximum n_agents and n_enemies
        ...
        # Actions
        self.n_actions_no_attack = 6        # no-operation, stop, north, south, east, west
        self.n_actions_move = 4             # north, south, east, west
        ...
        # Map info
        self.shield_bits_ally = 1 if self._agent_race == sc_common.Protoss else 0       # Here all ally units and enemy units should be the same family, such as Protoss.
        self.shield_bits_enemy = 1 if self._bot_race == sc_common.Protoss else 0
        if len(self.unit_types) > 1:
            self.unit_type_bits = len(self.unit_types)      # Self.unit_type_bits should be more than 1 if there are more than one type, otherwise 0.
        else:
        ...
        # its data)
        self.baneling_id = 1970
        self.colossus_id = 1971
        self.hydralisk_id = 1972
        self.marauder_id = 1973
        self.marine_id = 1974
        self.medivac_id = 1975
        self.stalker_id = 1976
        self.zealot_id = 1977
        self.zergling_id = 1978

        # 2022-11-17, add for random permutation
    ...
    def permutate_idx(self):
        self.agent_permutation = np.random.permutation(self.max_n_agents)       # Only randomly permutate the agent ordering
        enemies_order = np.arange(self.max_n_agents, (self.max_n_agents + self.max_n_enemies))
        self.entity_order = np.concatenate([self.agent_permutation, enemies_order])
        self.agent_recovery = [np.where(self.agent_permutation == i)[0][0] for i in range(self.max_n_agents)]
    ...
    def _calc_distance_mtx(self):
        ...
        dist_mtx = 1000 * np.ones((self.n_agents + self.n_enemies, self.n_agents + self.n_enemies))     # Un-observable mask is 1, otherwise equals to 0.
        for i in range(self.n_agents + self.n_enemies):
            for j in range(self.n_agents + self.n_enemies):
                if j < i:
                    continue            # this part is made up with the operation below (dist_mtx[i, j] = dist_mtx[j, i])
                elif j == i:
                    dist_mtx[i, j] = 0.0
                else:
                    if i >= self.n_agents:
                        ...
                        unit_a = self.agents[i]     # Select the target unit to get its pos info.
                    if j >= self.n_agents:
    ...
    def reset(self, unit_override=None, test=False, index=None, evaluate=False):
        ...
        # In the func init_units(), the global variables: task_index is updated in time.
        try:
            self.init_units(unit_override=unit_override, index=index, evaluate=evaluate)       # In this function, self.n_agents and self.n_enemies must be refreshed.
        except (protocol.ProtocolError, protocol.ConnectionError):
            self.full_restart()
            self.init_units(unit_override=unit_override, index=index, evaluate=evaluate)
        ...
        self._calc_distance_mtx()       # Update the distance_matrix

        if self.entity_scheme:
            global_states = self.get_entities()         # len(global_states)=max_n_agents+max_n_entities
            # full_obs_mask.shape=(max_n_agents+max_n_entities, max_n_agents+max_n_entities), scenario_mask.shape=(max_n_agents+max_n_entities)
            full_obs_mask, scenario_mask = self.get_masks()
            avail_actions = self.get_avail_actions()

            # TODO: 这里需要根据self.agent_permutation来返回修改后的global_states, full_obs_mask, scenario_mask以及avail_actions信息
            # 由于我们的算法并不适用local_obs，所以在parallel中需要单独设置一个env_random_smac的worker
            global_states = np.array(global_states)[self.entity_order]
            full_obs_mask = full_obs_mask[self.entity_order]
            scenario_mask = scenario_mask[self.entity_order]
            avail_actions = np.array(avail_actions)[self.agent_permutation]

            return global_states, (full_obs_mask, scenario_mask), avail_actions, self.task_index
        else:
            raise Exception("Only support entity_scheme in sc2custom")
    ...
    def step(self, actions, render_fn=None):
        ...
        # 2022-11-17, correct the original decision order
        ...
        if not step_success:
            # 2022-11-17 add for random permutation
            next_global_states = self.get_entities()  # (max_n_agents+max_n_enemies, local_state_size)
            # full_obs_mask.shape=(max_n_agents+max_n_entities, max_n_agents+max_n_entities), scenario_mask.shape=(max_n_agents+max_n_entities)
            full_obs_mask, scenario_mask = self.get_masks()
            avail_actions = self.get_avail_actions()  # (max_n_agents, n_actions)

            # TODO: 根据self.agent_permutation实现global_states, obs_mask, scenario_mask以及available actions的permutation
            # terminated 以及 info 是否不需要permutation?
            next_global_states = np.array(next_global_states)[self.entity_order]
            full_obs_mask = full_obs_mask[self.entity_order]
            scenario_mask = scenario_mask[self.entity_order]
            avail_actions = np.array(avail_actions)[self.agent_permutation]

            return next_global_states, (full_obs_mask, scenario_mask), avail_actions, 0, True, {}
        ...
        if render_fn is None:
            return next_global_states, (full_obs_mask, scenario_mask), avail_actions, reward, terminated, info
        else:
            return next_global_states, (full_obs_mask, scenario_mask), avail_actions, reward, terminated, info, frames
    ...
    def get_masks(self):
        """
        Returns:
        1) obs mask: per agent observability mask over all entities (unoberserved = 1, else 0)
        2) entity mask: mask of inactive entities (including enemies) over all possible entities
        """
        sight_range = np.array(
            [self.unit_sight_range(a_i)
             for a_i in range(self.n_agents + self.n_enemies)]).reshape(-1, 1)
        obs_mask = (self.dist_mtx > sight_range).astype(np.uint8)       # un-observable is 1, otherwise is 0
        ...
        return obs_mask_padded, entity_mask     # This definition is the same as ones in our custom mpe scenarios.

    def get_entities(self):
        """
        Returns list of agent entities and enemy entities in the map (all entities are a fixed size). Shape=(max_n_agents+max_n_enemies, nf_entity)
        ...
        max_dist_com = max(self.distance(unit.pos.x, unit.pos.y, com_x, com_y)      # The maximum acts like detecting the outliers, the furthest agent's distance to the center of agents mass
        ...
        for u_i, unit in enumerate(all_units):
            ...
            if u_i < self.n_agents:
                for ac_i in range(self.n_actions - 2):
                    entity[ind + ac_i] = avail_actions[u_i][2 + ac_i]       # The first two actions are available all the time.
    ...
    # def init_units(self, unit_override=None, index=None, evaluate=False):
    #     """Initialise the units."""
    #     while True:
    #         self._kill_all_units()
    #
    #         self.n_agents = 0
    #         if not self.max_reward_init:            # Choose the scenario with the maximum allies and enemies to intialize somethings.
    #             scenario = self.max_types_and_units_scenario
    #         elif index is not None:                 # Pre-define the index of the selected scenario.
    #             scenario = self.scenarios[index]
    #         else:                                   # Randomly select one.
    #             scenario = self.scenarios[self.rs.randint(len(self.scenarios))]
    #         armies = self._assign_pos(scenario)
    #
    #         # 20221112 add the task index setting
    #         task_index = self.scenarios.index(scenario)
    #         curr_task_index = np.zeros(len(self.scenarios))
    #         curr_task_index[task_index] = 1
    #         self.task_index = curr_task_index
    #
    ...
    #                     quantity=num))
    #             cmds.append(cmd)
    #             self.n_agents += num            # Refresh the number of allies.
    #
    #         self.n_enemies = 0
    ...
    #                     quantity=num))
    #             cmds.append(cmd)
    #             self.n_enemies += num           # Also refresh the number of enemies.
    ...
    #                 if unit.owner == 2:         # Only uses the info of enemies [whose owner is 2] to calculate the self.max_reward
    #                     self.max_reward += unit.health_max + unit.shield_max
    #         self.max_reward_init = True
    #
    #     self.agents = {}            # Its info contains all allies' unit_type, pos.x and pos.y
    #     self.enemies = {}
    #
    ...
    # 20221112 modified version (add task_index, sample tasks from training scenarios when evaluate is False.)
    def init_units(self, unit_override=None, index=None, evaluate=False):
        """Initialise the units."""
        while True:
            self._kill_all_units()
            ...
            if not self.max_reward_init:            # Choose the scenario with the maximum allies and enemies to intialize somethings.
                ...
                if evaluate:
                    scenario = self.testing_tasks[self.rs.randint(len(self.testing_tasks))]
                else:
                    scenario = self.training_tasks[self.rs.randint(len(self.training_tasks))]
    ...
    def _init_enemy_strategy(self):
        ...
        cmd = r_pb.ActionRawUnitCommand(            # This cmd means that go to the ally_spawn_center and attack the allies? Remain to be validated with live video.
    ...
    def get_env_info(self, args):
        global_state_size = self.get_entity_size() * (self.max_n_agents + self.max_n_enemies)
        if self.entity_scheme:
            env_info = {"state_shape": global_state_size,
                        "obs_shape": global_state_size,
                        "n_actions": self.get_total_actions(),
                        "n_agents": self.max_n_agents,
                        "n_entities": self.max_n_agents + self.max_n_enemies,
                        "episode_limit": self.episode_limit,
                        "n_tasks": len(self.scenarios)}
        else:
            raise Exception("Un-supported env setting except for entity_scheme.")
        return env_info